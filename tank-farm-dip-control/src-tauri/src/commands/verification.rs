...
fn finalize_recheck_relation(
    ...
    let relation: Option<(i64, i64)> = conn
        .query_row(
    ...
    if let Some((relation_id, original_id)) = relation {
        conn.execute(
...
pub fn review_dip(
    ...
    match action.as_str() {
        "approve" => {
            let exception_count = open_exception_count(&conn, dip_id);
            if exception_count > 0 && remarks.as_deref().map(str::trim).unwrap_or("").is_empty() {
                return Err("Approval remarks are required when tolerance exceptions are open".to_string());
            }
        ...
        "recheck" => {
            ...
            conn.execute(
                "UPDATE dip_rechecks SET final_decision='recheck_again', reviewer_id=?1
                 WHERE recheck_dip_id=?2 AND COALESCE(final_decision, 'pending')='pending'",
                params![session.user_id, dip_id],
            )
            .map_err(|e| e.to_string())?;
        }
        "reject" => {
            ...
            let parent: Option<(i64, i64)> = conn
                .query_row(
                    "SELECT id, original_dip_id FROM dip_rechecks WHERE recheck_dip_id=?1 AND COALESCE(final_decision,'pending')='pending' ORDER BY id DESC LIMIT 1",
                    params![dip_id],
                    |row| Ok((row.get(0)?, row.get(1)?)),
                )
                .ok();
            if let Some((relation_id, original_id)) = parent {
                conn.execute(
    ...
    conn.query_row(
        "SELECT id, dip_record_id, reviewer_id, review_action, review_remarks, reviewed_at
         FROM dip_reviews WHERE id=?1",
        params![review_id],
        |row| {
            Ok(DipReview {
                id: row.get(0)?,
...
pub fn recheck_dip(
    ...
    conn.execute(
        ...
        params![
            ...
            new_readings.date,
            new_readings.time,
            new_readings.shift_id,
            new_readings.tank_id,
            new_readings.product_id,
            reference_point_snapshot,
            new_readings.gross_dip_mm,
            new_readings.auto_dip_mm,
            new_readings.radar_dip_mm,
            new_readings.water_dip_mm,
            new_readings.sludge_dip_mm,
            new_readings.temperature,
            new_readings.temperature_unit,
            new_readings.density,
            new_readings.tank_status_id,
            new_readings.custom_tank_status,
            new_readings.operator_id,
            new_readings.remarks,
            gross_auto,
            gross_radar,
            auto_radar,
            session.user_id,
    ...
    audit_log(
        ...
        Some(&record_number),
        remarks.as_deref(),
        Some("Physical recheck Dip recorded and submitted for final review"),
    );
...
pub fn approve_recheck(
    ...
    if open_exception_count(&conn, dip_id) > 0 {
        return Err("This recheck has open exceptions. Use normal Approve with review remarks or request another recheck.".to_string());
    }

    let relation: DipRecheck = conn
        .query_row(
            "SELECT id, original_dip_id, recheck_dip_id, recheck_operator_id, recheck_remarks, reviewer_id, final_decision, created_at
             FROM dip_rechecks WHERE recheck_dip_id=?1 AND COALESCE(final_decision,'pending')='pending' ORDER BY id DESC LIMIT 1",
            params![dip_id],
            |row| {
                Ok(DipRecheck {
                    id: row.get(0)?,
        ...
        .map_err(|_| "Pending recheck relation not found".to_string())?;
    ...
    audit_log(
        ...
        None,
        None,
        Some("Recheck Dip approved; original Dip superseded"),
    );
...
fn apply_correction(conn: &rusqlite::Connection, correction: &DipCorrection) -> Result<(), String> {
    ...
    match correction.field_name.as_str() {
        "gross_dip_mm" | "auto_dip_mm" | "radar_dip_mm" | "water_dip_mm" | "sludge_dip_mm" | "temperature" | "density" => {
            let parsed: f64 = value.parse().map_err(|_| format!("Invalid numeric correction value for {}", correction.field_name))?;
            if !parsed.is_finite() || parsed < 0.0 {
                return Err(format!("Invalid correction value for {}", correction.field_name));
            }
        ...
        "tank_status_id" | "operator_id" | "product_id" => {
            let parsed: i64 = value.parse().map_err(|_| format!("Invalid integer correction value for {}", correction.field_name))?;
            let sql = format!("UPDATE dip_records SET {}=?1 WHERE id=?2", correction.field_name);
            conn.execute(&sql, params![parsed, id]).map_err(|e| e.to_string())?;
        }
        "temperature_unit" => {
            if value != "C" && value != "F" {
                return Err("Temperature unit correction must be C or F".to_string());
            }
            conn.execute("UPDATE dip_records SET temperature_unit=?1 WHERE id=?2", params![value, id])
                .map_err(|e| e.to_string())?;
        }
...
pub fn approve_correction(
    ...
    if correction.status != "pending" {
        return Err("Correction request is not pending".to_string());
    }

    let record = crate::commands::dips::get_dip_record_by_id_raw(&conn, correction.dip_record_id)?;
    if record.record_status != "correction_requested" || record.approval_status != "correction_pending" {
        return Err("Dip Record is not in correction approval workflow".to_string());
    }

    apply_correction(&conn, &correction)?;
    conn.execute(
        "UPDATE dip_corrections SET approved_by=?1, status='approved' WHERE id=?2",
        params![session.user_id, correction_id],
    )
    .map_err(|e| e.to_string())?;
    ...
    conn.execute(
        "UPDATE dip_records SET gross_auto_difference=?1, gross_radar_difference=?2, auto_radar_difference=?3 WHERE id=?4",
        params![gross_auto, gross_radar, auto_radar, correction.dip_record_id],
    )
    .map_err(|e| e.to_string())?;

    let pending: i64 = conn
        .query_row(
            "SELECT COUNT(*) FROM dip_corrections WHERE dip_record_id=?1 AND status='pending'",
            params![correction.dip_record_id],
            |row| row.get(0),
        )
        .unwrap_or(0);
    ...
    if pending == 0 {
        ...
        conn.execute(
            "UPDATE dip_records SET review_status='approved', approval_status='approved', record_status='approved',
             approved_by=?1, approved_at=?2 WHERE id=?3",
            params![session.user_id, now, correction.dip_record_id],
        )
        .map_err(|e| e.to_string())?;
    }
    ...
    audit_log(
        ...
        Some(&format!("Correction approved for {}", correction.field_name)),
    );
...
pub fn get_pending_reviews(
    ...
    let conn = db.lock().map_err(|e| e.to_string())?;
    let sql = format!(
        "{} WHERE dr.record_status='submitted' AND dr.review_status IN ('pending','recheck_pending') ORDER BY dr.date DESC, dr.time DESC",
        base_related_sql()
    );
    let mut stmt = conn.prepare(&sql).map_err(|e| e.to_string())?;
    let rows = stmt
        .query_map([], query_related)
        .map_err(|e| e.to_string())?
...
pub fn list_dip_records_with_relations(
    ...
    if let Some(value) = filters.date_from {
        sql.push_str(&format!(" AND dr.date>=?{}", values.len() + 1));
    ...
    if let Some(value) = filters.shift_id {
        sql.push_str(&format!(" AND dr.shift_id=?{}", values.len() + 1));
        values.push(Box::new(value));
    }
    if let Some(value) = filters.tank_id {
        sql.push_str(&format!(" AND dr.tank_id=?{}", values.len() + 1));
        values.push(Box::new(value));
    }
    if let Some(value) = filters.review_status {
        sql.push_str(&format!(" AND dr.review_status=?{}", values.len() + 1));
        values.push(Box::new(value));
    }
    if let Some(value) = filters.approval_status {
        sql.push_str(&format!(" AND dr.approval_status=?{}", values.len() + 1));
        values.push(Box::new(value));
    }
    if let Some(value) = filters.record_status {
        sql.push_str(&format!(" AND dr.record_status=?{}", values.len() + 1));
        values.push(Box::new(value));
    }
    if let Some(value) = filters.operator_id {
        sql.push_str(&format!(" AND dr.operator_id=?{}", values.len() + 1));
        values.push(Box::new(value));
    }
    ...
    let mut stmt = conn.prepare(&sql).map_err(|e| e.to_string())?;
    let rows = stmt
        .query_map(refs.as_slice(), query_related)
        .map_err(|e| e.to_string())?
        .collect::<Result<Vec<_>, _>>()
        .map_err(|e| e.to_string())?;
    Ok(rows)
