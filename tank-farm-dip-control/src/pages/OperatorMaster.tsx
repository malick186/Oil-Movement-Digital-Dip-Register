  ...
  return (
    <div className="h-full flex flex-col">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-base font-semibold text-slate-700">Operator Master</h2>
        <button onClick={() => { setEditingId(null); reset({ employee_id: '', name: '', designation: '', location: '', shift_group: '', active: true, remarks: '' }); setShowForm(true); }} className="flex items-center gap-1 bg-blue-600 hover:bg-blue-700 text-white text-xs px-2 py-1 rounded">
      ...
      {showForm && (
        <div className="bg-white rounded border border-slate-200 p-4 mb-4">
          <h3 className="text-sm font-medium text-slate-700 mb-3">{editingId ? 'Edit Operator' : 'New Operator'}</h3>
          <form onSubmit={handleSubmit(onSubmit)}>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              <div>
                <label className="block text-[11px] font-medium text-slate-600 mb-1">Employee ID <span className="text-red-500">*</span></label>
              ...
              <div>
                <label className="block text-[11px] font-medium text-slate-600 mb-1">Shift Group</label>
      ...
      <div className="bg-white rounded border border-slate-200 overflow-auto flex-1">
        <table className="w-full text-[11px]">
          <thead className="bg-slate-50 border-b border-slate-200 sticky top-0">
            <tr>
              <th className="text-left px-2 py-1.5 font-medium text-slate-600">Emp ID</th>
              <th className="text-left px-2 py-1.5 font-medium text-slate-600">Name</th>
              <th className="text-left px-2 py-1.5 font-medium text-slate-600">Designation</th>
              <th className="text-left px-2 py-1.5 font-medium text-slate-600">Location</th>
              <th className="text-left px-2 py-1.5 font-medium text-slate-600">Shift Group</th>
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={7} className="text-center py-8 text-slate-400">No operators found</td></tr>
            ) : (
              operators.map((o) => (
                <tr key={o.id} className="border-b border-slate-100 hover:bg-slate-50">
                  <td className="px-2 py-1 font-mono text-slate-700">{o.employee_id}</td>
                  <td className="px-2 py-1 font-medium text-slate-600">{o.name}</td>
                  <td className="px-2 py-1 text-slate-500">{o.designation || '--'}</td>
                  <td className="px-2 py-1 text-slate-500">{o.location || '--'}</td>
                  <td className="px-2 py-1 text-slate-500">{o.shift_group || '--'}</td>
                  <td className="px-2 py-1 text-center">{o.active ? <span className="text-green-600">Yes</span> : <span className="text-red-500">No</span>}</td>
                  <td className="px-2 py-1 text-center">
                    <div className="flex gap-1 justify-center">
                      <button onClick={() => handleEdit(o)} className="p-0.5 text-slate-400 hover:text-blue-600"><Pencil size={13} /></button>
                      <button onClick={async () => { try { await api.deleteOperator(o.id); loadOperators(); } catch {} }} className="p-0.5 text-slate-400 hover:text-red-600"><Trash2 size={13} /></button>
                    </div>
                  </td>
                </tr>
              ))
